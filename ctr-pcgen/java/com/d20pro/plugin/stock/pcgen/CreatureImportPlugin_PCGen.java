...
public class CreatureImportPlugin_PCGen implements ImportCreaturePlugin, XMLToDocumentHelperStrategy
{
  ...
  public java.util.List<CreatureTemplate> parseCreatures(CreatureImportServices svc, ImportMessageLog log, Document doc)
      ...
      throws ImportCreatureException
  {
    ...
    ctr.setNotes(
            "Imported from PCGen (" + gameSystem + ") on " + DateFormat.getDateInstance().format(new Date()) + "\n");
  ...
  private static void parseBasics(CreatureTemplate ctr, Node node) throws ImportCreatureException
  {
    ...
    try {
      ctr.setSize((byte) Rules.getInstance().invokeMethod("Rules.Size.getID", new Object[] { size } ) );
    } catch (Exception e) {
      //TODO check handling
      throw new ImportCreatureException("missing class or file. "+e.getMessage());
    }
  ...
  private static void parseAttributes(CreatureTemplate ctr, Node node) throws ImportCreatureException
  {
    ...
    try {
      ...
      throw new ImportCreatureException("missing name. "+e.getMessage());
    }
    ...
    for (byte i = 0; i < NAMES.length; i++)
    {
      ...
      //TODO change setAbilityScore to AbilityScoreBase
      ctr.setAbilityScore(i, (byte) score);
    }
  }
